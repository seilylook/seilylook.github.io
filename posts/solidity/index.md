...
### 6.4 Contract 함수 호출하기

이제는 Web3.js로 컨트랙트와 통신할 수 있다.

Web3.js는 Contract의 함수를 호출하기 위해 우리가 사용할 두 개의 메소드를 가지고 있다: `call`과 `send`이다.

#### Call

`call`은 `view`와 `pure` 함수를 위해 사용한다. 로컬 노드에서만 실행하고, 블록체인에 Transaction을 만들지 않는다.

{{<admonition note>}}
복습: `view`와 `pure` 함수는 읽기 전용이고 블록체인에서 상태를 변경하지 않네. 가스를 전혀 소모하지 않고, 메타마스크에서 트랜잭션에 서명하라고 사용자에게 창을 띄우지도 않지.
{{</admonition>}}

Web3.js를 사용해, 다음과 같이 `123`을 매개 변수로 `myMethod`라는 이름의 함수를 `call`할 수 있다.
...
`send`는 transaction을 만들고 블록체인 상의 데이터를 변경한다. `view`와 `pure`가 아닌 모든 함수에 대해 `send`를 사용해야 하는 것이다.
...
Web3.js를 사용해, 다음과 같이 `123`을 매개 변수로 `myMethod`라는 이름의 함수를 `send`할 수 있다.

### 6.5 Metamask & 계정

우리 앱의 홈페이지에 사용자의 전체 좀비 군대를 보여주고 싶다고 가정해보자.

현재 사용자가 가지고 있는 모든 좀비들의 ID를 찾기 위해 우리는 분명 첫 번째로 `getZombiesByOwner(owner)` 함수를 사용해야 한다.

하지만 우리의 Solidity Contract는 `owner`에 Solidity `address`를 보내도록 되어 있다. 그렇다면 우리가 어떻게 우리 앱을 사용하는 사용자의 주소를 알 수 있는가?

#### Metamask에서 사용자 계정 가져오기

Metamask는 확장 프로그램 안에서 사용자들이 다수의 계정을 관리할 수 있도록 해준다.
...
사용자가 언제든지 Metamask에서 활성홛된 계정을 바꿀 수 있기 때문에, 우리 앱은 이 변수의 값이 바뀌었는지 확인하기 위해 계속 감시를 하고 값이 바뀌면 그에 따라 UI를 업데이트해야 한다. 예를 들어, 사용자들의 홈페이지에서 그들의 좀비 군대를 표시하고 싶다면, Metamask에서 계정을 바꾸었을 때 바뀐 계정에 대한 좀비 군대를 보여주기 위해 페이지들을 업데이트 해야한다.

예를 들어 다음과 같이 `setInterval`을 쓸 수 있다.
...
여기서는 `userAccount`가 여전히 `web3.eth.accounts[0]`과 같은지 확인하기 위해 100밀리초마다 확인하고 있다(즉, 사용자가 해당 계정을 활성화해놓았는지 확인하는 것이다). 그렇지 않다면, `userAccount`에 현재 활성화된 계정을 다시 할당하고, 화면을 업데이트하기 위한 함수를 호출한다.

### 6.6 Transaction 보내기

이제 우리의 UI는 사용자의 Metamask 계정을 감지하고, 자동으로 좀비 군대를 홈페이지에 표현할 것이다.

이제 `send` 함수를 이용해 Smart Contract의 데이터를 변경하는 방법을 알아본다.

이 함수에는 `call` 함수와는 꽤 다른 부분이 있다.

- Transaction을 전송(`send`)하려면 함수는 호출한 사람의 `from` 주소가 필요하다(Solidity 코드에서는 `msg.sender`가 될 것이다). 이는 우리 DApp의 사용자가 되어야 할 것이니, Metamask가 나타나 그들에세 서명을 하도록 한다.

2. Transaction `send`은 가스를 소모한다.

3. 사용자가 Transaction 전송을 하고 난 후 실제로 블록체인에 적용될 때까지는 상당한 지연이 발생할 것이다. Transaction이 블록에 포함될 때까지 기다려야 하는데, Ethereum의 평균 블록 시간이 15초이기 때문이다. 만약 Ethereum에 보유 중인 거래가 많거나 사용자가 가스 가격을 지나치게 낮게 보낼 경우, 우리 Transaction이 블록에 포함되길 기다려야 하고, 이는 몇 분씩 걸릴 수 있다.

그러니 이 코드의 비동기적 특성을 다루기 위한 로직이 필요하다.

#### 좀비 만들기

이제 사용자가 호출할 우리 Contract 내의 첫번째 함수를 예제로 살펴본다: `createRandomZombie`

기존에 만들어 둔 Solidity 코드를 다시 확인해본다.
...
다음은 Metamask를 사용해 Web3.js에서 위 함수를 호출하는 방법이다.
...
위 함수는 우리의 Web3 Provider에게 Transaction을 `send`하고, 몇 가지 이벤트 리스너를 연결한다.
